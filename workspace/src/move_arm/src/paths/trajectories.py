...
class LinearTrajectory(Trajectory):
    def __init__(self, start_position, goal_position, total_time):
        ...
        self.desired_orientation = np.array([0, np.sqrt(1/2), 0, np.sqrt(1/2)])
    ...
    def target_pose(self, time):
        ...
        if time <= self.total_time / 2.0:
            ...
            pos = (self.goal_position+self.start_position)/2 + (self.v_max*time - 0.5*self.acceleration*time**2)
    ...
    def target_velocity(self, time):
        ...
        if time <= self.total_time / 2.0:
            ...
            # TODO: start slowing the velocity down from the maximum oneC
...
class CircularTrajectory(Trajectory):
    ...
    def target_pose(self, time):
        ...
        7D vector [x, y, z, qx, qy, qz, qw]. i.e. the first three entries are 
        the desired end-effector position, and the last four entries are the 
        desired end-epython main.py -task {line/circle} -ar_marker {number} --logffector orientation as a quaternion, all written in the 
        world frame.
        ...
        if time <= self.total_time / 2.0:
            ...
            time = time - self.total_time / 2.0
            theta = np.pi + (self.angular_v_max*time - 0.5 * self.angular_acceleration * time**2)
